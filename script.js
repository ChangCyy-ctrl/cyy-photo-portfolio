...
const fixedHeroCoverItem = {
  category: "city",
  name: "微信图片_20260228212620_153_277.jpg",
};
...
function setHeroCoverFromValid() {
  ...
  const fixedCoverPath = buildPath(fixedHeroCoverItem);
  const probe = new Image();
  probe.onload = () => {
    hero.style.backgroundImage = `url("${fixedCoverPath}")`;
  };
  probe.onerror = () => {
    const cityCover = displayPhotos.find((p) => p.category === "city") || displayPhotos[0];
    hero.style.backgroundImage = `url("${buildPath(cityCover)}")`;
  };
  probe.src = fixedCoverPath;
}
